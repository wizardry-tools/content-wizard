...
(() => {
      ...
      return (stream: CodeMirror.StringStream, state: SqlState) => {
        ...
        while ((ch = stream.next()) != null) {
          if (ch === quote && !escaped) {
            ...
            break;
          } else if (ch === endToken && !escaped) {
            // handles [bracketedVariables]
            variable = true;
            state.tokenize = tokenBase;
            break;
          }
  ...
  CodeMirror.defineMIME('text/x-jcrsql2', {
    ...
    dateSQL: set('date time timestamp'),
    support: set(
      'bracketVariable decimallessFloat zerolessFloat binaryNumber hexNumber doubleQuote nCharCast charsetCast commentHash commentSpaceRequired',
    ),
    hooks: {
      '\\': hookClient,
    },
