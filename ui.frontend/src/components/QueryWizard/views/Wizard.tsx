...
import {useFields, useFieldDispatch, useQuery} from "../providers/QueryProvider";
...
import {QueryWizardViewsProps} from "../QueryWizardViews";
...
type WizardProps = Pick<QueryWizardViewsProps, 'onTabPanelSelect'>;

export default function Wizard({onTabPanelSelect}: WizardProps) {
  ...
  const Tab = useCallback((accordionFields: FieldConfig[]) => {
          ...
          <Tooltip.Provider>
            <Stack className="wizard-editor-stack" direction="row" spacing={1}>
              <WizardStatementEditor
                editorTheme="wizard"
                keyMap="sublime"
                className="querybuilder-statement"
              />
            </Stack>
          </Tooltip.Provider>
        </Paper>
  ...
  return(
    <Paper className="wizard-builder">
      ...
      >
        <Stack className="statement-stack">
          {StatementWindow}
        </Stack>
